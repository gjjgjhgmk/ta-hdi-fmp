"""hdi.py - 自适应高密度插值 (Adaptive High Density Interpolation)

对应 MATLAB: func/adaptive_hdi.m
...
def adaptive_hdi(
    ...
    Args:
        rrt_path: RRT 路径 (2, N)
        obstacles: 障碍物数组 (M, 6)
        cfg: PlannerConfig 配置对象
        idx_start: 起始点在原始示教轨迹中的索引
        idx_goal: 目标点在原始示教轨迹中的索引
        demo_dt: 示教时间步

    Returns:
        dense: 加密后的路径点 (2, M)
        t_local: 对应的时间参数 (M,)
        danger_mask: 危险区域标记 (M,)
    """
